...
export default function HomePage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-background px-6 py-6 text-foreground sm:px-10 sm:py-8">
      {!ready ? <LoadingAnimation onComplete={() => setReady(true)} /> : null}

      <div className={`mx-auto flex min-h-[calc(100vh-3rem)] max-w-6xl flex-col transition-opacity duration-200 ${ready ? "opacity-100" : "opacity-0"}`}>
        <header className="flex items-start justify-between text-sm">
          <div className="font-display text-lg uppercase tracking-caps">Sutor</div>
          ...
          <div className="mt-14 grid w-full max-w-4xl gap-4 md:grid-cols-3">
            {instruments.map((instrument) => (
              <button
                ...
                className="group border border-border bg-surface px-8 py-12 text-center transition-colors duration-150 hover:border-accent"
              >
                <span className="font-display text-3xl capitalize">{instrument}</span>
              </button>
            ))}
          </div>
        </section>
      ...
      {settingsOpen ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-[rgba(248,248,246,0.9)] px-6">
